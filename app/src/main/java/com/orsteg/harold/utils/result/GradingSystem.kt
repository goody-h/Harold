...
class GradingSystem(private val context: Context) {
    ...
    val gradeStates: ArrayList<Boolean>
        get() {
            return (0..18).mapTo(ArrayList()) { gradeStore.mPrefs.getBoolean("grading.gradeState$it", DEFAULT_GRADE_STATES[it]) }
        }

    val allGrades: ArrayList<String>
        get() {
            return (0..18).mapTo(ArrayList()) { gradeStore.mPrefs.getString("grading.grade$it", DEFAULT_GRADES[it]) }
        }

    val allGradePoints: ArrayList<Float>
        get() {
            return (0..18).mapTo(ArrayList()) { gradeStore.mPrefs.getFloat("grading.gradePoint$it", DEFAULT_GRADE_POINTS[it]) }
        }
    ...
    companion object {
        ...
        private val DEFAULT_GRADES = arrayOf("A", "AB", "A+", "A-", "B+", "B", "BC", "B-", "C+", "C", "CD", "C-", "D+", "D", "D-", "E", "F", "NC", "U")

        private val DEFAULT_GRADE_POINTS = floatArrayOf(5.0f, 3.5f, 4.0f, 3.7f, 3.3f, 4.0f, 3.0f, 2.7f, 2.3f, 3.0f, 2.5f, 1.7f, 1.0f, 2.0f, 0.7f, 1.0f, 0.0f, 0.0f, 0.0f)

        private val DEFAULT_GRADE_STATES = booleanArrayOf(true, false, false, false, false, true, false, false, false, true, false, false, false, true, false, true, true, false, false)
    }
}
